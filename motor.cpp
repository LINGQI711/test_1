...
void motor_step(int left, int right, int step_left, int step_right) {
  ...
  {
    if (left_velocity > abs(left_tar)) pwm_l > 0 ? pwm_l-- : pwm_l++;
    else left_tar > 0 ? pwm_l++ : pwm_l--;
    if (right_velocity > abs(right_tar)) pwm_r > 0 ? pwm_r-- : pwm_r++;
    else right_tar > 0 ? pwm_r++ : pwm_r--;
...
void GoStraight(int setspeed = 160) {
  //setspeed = -200;
  int left = setspeed + 10;
  int right = setspeed;
  int step_left = 1000;
  int step_right = 1000;
  static int left_tar = 0, right_tar = 0;
  ...
  } else if ((left_tar || right_tar) && (step_left || step_right))  //设定速度且指定距离
  {
    ...
    if (!left_tar)  //左侧不转
      ...
      while (right_pulse - t_pulse_r <= (unsigned int)step_right || left_pulse - t_pulse_l <= (unsigned int)step_left) {
          ...
          int right_steps = right_pulse - t_pulse_r;
          int left_steps = left_pulse - t_pulse_l;
          //Serial.print(left_steps);
          //Serial.println(right_steps);
          if(left_steps - right_steps > 1)
          {
            int newleft = left + (left_steps - right_steps);
             motor_step(newleft, right);
            //Serial.print("左快,修正：");
            //Serial.print(newleft);
          //Serial.println(right);

          }
          else if (left_steps - right_steps < -1) {
            int newleft = left + (left_steps - right_steps);
             motor_step(newleft, right);
            //Serial.print("左慢,修正：");
            //Serial.print(newleft);
